...
export default function ListaAlumnosPage() {
    ...
    const renderDetalleDesktop = (alumno: any) => {
        ...
        const infoFields = [
            ...
            { label: 'Horario', value: capitalizar(alumno.horarioEntrenamiento) },
            { label: 'Días/sem', value: alumno.diasEntrenaSemana ? `${alumno.diasEntrenaSemana} días` : '-' },
            ...(alumno.area ? [{ label: 'Área', value: areaLabel[alumno.area] ?? alumno.area }] : []),
            ...(alumno.nivelExperiencia ? [{ label: 'Nivel', value: nivelLabel[alumno.nivelExperiencia] ?? alumno.nivelExperiencia }] : []),
            ...(alumno.patologias ? [{ label: 'Patologías', value: alumno.patologias }] : []),
        ];
        return (
            <div className="bg-white border border-black/[0.07] rounded-2xl shadow-sm overflow-hidden">
                <div className="bg-[#111] px-6 py-4 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center shrink-0">
                        <span className="text-white font-bold text-lg">{initials}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-white font-bold text-lg leading-tight">{alumno.apellido}, {alumno.nombre}</p>
                        <p className="text-slate-400 text-sm mt-0.5">DNI {alumno.dni}</p>
                    </div>
                    <span className={`text-sm font-bold px-3 py-1.5 rounded-full ${pagado ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                        {pagado ? 'Pagó' : 'Debe'}
                    </span>
                    <button onClick={() => setAlumnoSeleccionado(null)}
                        className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors ml-1" aria-label="Cerrar">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
                        </svg>
                    </button>
                </div>
                <div className="px-6 py-4 flex flex-wrap gap-8 border-b border-slate-100">
                ...
                <div className="px-6 py-4 flex items-center gap-3 flex-wrap">
                    <button onClick={() => router.push(`/alumnos/${alumno._id}/historial`)}
                        className="flex items-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white text-sm font-semibold rounded-xl transition-all active:scale-95">
                    ...
                    <button onClick={() => marcarPagoMes(alumno._id)}
                        className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold rounded-xl transition-all active:scale-95">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75" /></svg>
                        Marcar Pago
                    </button>
                    <button onClick={() => iniciarPlan(alumno._id)}
                        className="flex items-center gap-2 px-4 py-2.5 bg-amber-500 hover:bg-amber-400 text-white text-sm font-semibold rounded-xl transition-all active:scale-95">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z" /></svg>
                        Iniciar Plan
                    </button>
                    <button onClick={() => handleEditarAlumno(alumno)}
                        className="flex items-center gap-2 px-4 py-2.5 bg-[#111] hover:bg-zinc-800 text-white text-sm font-semibold rounded-xl transition-all active:scale-95">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" /></svg>
                        Editar
                    </button>
                    {['dueño', 'admin'].includes(session?.user?.role ?? '') && (
                        <button onClick={() => handleResetPassword(alumno)}
                            className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold rounded-xl transition-all active:scale-95">
                    ...
                    <button onClick={() => eliminarAlumno(alumno._id)}
                        className="ml-auto flex items-center gap-2 px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 text-sm font-semibold rounded-xl transition-all active:scale-95">
    ...
    const renderDetalle = (alumno: any) => {
        const pagado = verificarPagoMesActual(alumno.pagos);
        const initials = `${alumno.nombre?.[0] ?? ''}${alumno.apellido?.[0] ?? ''}`.toUpperCase();
        return (
            <div className="relative bg-white rounded-3xl overflow-hidden shadow-sm border border-black/[0.07]">
                <div className="bg-[#111] px-5 pt-5 pb-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-white font-bold text-base leading-tight truncate">{alumno.apellido}, {alumno.nombre}</p>
                            <p className="text-slate-400 text-xs mt-0.5">DNI {alumno.dni}</p>
                        </div>
                ...
                <div className="px-4 py-3 grid grid-cols-2 gap-2 border-b border-slate-100">
                    {[
                        ['Edad', alumno.edad ? `${alumno.edad} años` : '-'],
                        ['Teléfono', alumno.telefono || '-'],
                        ['Horario', capitalizar(alumno.horarioEntrenamiento)],
                        ['Días/sem', alumno.diasEntrenaSemana ? `${alumno.diasEntrenaSemana} días` : '-'],
                        ...(alumno.area ? [['Área', areaLabel[alumno.area] ?? alumno.area]] : []),
                        ...(alumno.nivelExperiencia ? [['Nivel', nivelLabel[alumno.nivelExperiencia] ?? alumno.nivelExperiencia]] : []),
                        ...(alumno.patologias ? [['Patologías', alumno.patologias]] : []),
                    ].map(([lbl2, value]) => (
                        <div key={lbl2} className={`bg-slate-50 rounded-xl px-3 py-2 ${lbl2 === 'Patologías' ? 'col-span-2' : ''}`}>
                            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">{lbl2}</p>
    ...
    return (
        <div className="max-w-7xl mx-auto pt-4 pb-12 px-4 lg:px-8">
            ...
            {/* Panel detalle desktop — aparece debajo del listado, full width */}
            {alumnoSeleccionado && (
                <div className="hidden lg:block mt-4">
                    {renderDetalleDesktop(alumnoSeleccionado)}
                </div>
            )}

            {/* Modal detalle alumno — solo mobile */}
            <Modal
                isOpen={!!alumnoSeleccionado}
                onRequestClose={() => setAlumnoSeleccionado(null)}
                contentLabel="Detalle del Alumno"
                className="relative w-full max-w-sm mx-auto mt-12 outline-none lg:hidden"
                overlayClassName="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-start z-50 px-4 overflow-y-auto pb-8 lg:hidden"
            >
                {alumnoSeleccionado && renderDetalle(alumnoSeleccionado)}
            </Modal>
        </div>
    );
}
