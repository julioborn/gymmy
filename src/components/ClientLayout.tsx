...
function LayoutWithSession({ children }: ClientLayoutProps) {
    ...
    const navItems = (() => {
        ...
        const miPerfil = {
            ...
            label: 'Mi perfil',
            isCenter: false,
            isActive: (p: string) => p.startsWith('/mi-perfil') && (typeof window === 'undefined' || !window.location.hash || window.location.hash === '#resumen' || window.location.hash === '#historial'),
            icon: (
                <svg className="w-[20px] h-[20px]" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
        ...
        const miPlan = {
            ...
            label: 'Mi plan',
            isCenter: false,
            isActive: (p: string) => p.startsWith('/mi-perfil') && typeof window !== 'undefined' && window.location.hash === '#plan',
            icon: (
                <svg className="w-[20px] h-[20px]" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
        ...
        return [alumnos, inicio, miPlan, miPerfil];
    })();
